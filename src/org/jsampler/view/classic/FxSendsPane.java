 ...
 * @author Grigor Iliev
 */
public class FxSendsPane extends JPanel implements ListSelectionListener {
	...
	private final JLabel lMidiCtrl = new JLabel(i18n.getLabel("FxSendsPane.lMidiCtrl"));
	private final JLabel lVolume = new JLabel(i18n.getLabel("FxSendsPane.lVolume"));
	...
	/**
	 * Creates a new instance of <code>FxSendsPane</code>.
	...
	FxSendsPane(SamplerChannelModel model) {
		...
		tb.add(btnNewFxSend);
		tb.add(btnRemoveFxSend);
		
		JPanel leftPane = new JPanel();
		leftPane.setLayout(new BorderLayout());
		
		leftPane.add(tb, BorderLayout.NORTH);
		...
		if(channelModel.getChannelInfo().getEngine() == null) {
			btnNewFxSend.setEnabled(false);
		}
		if(channelModel.getFxSendCount() == 0) {
			btnRemoveFxSend.setEnabled(false);
		} else {
	...
	valueChanged(ListSelectionEvent e) {
		...
		btnRemoveFxSend.setEnabled(fxSend != null);
		updateFxSend();
	}
	...
	class AddFxSend extends AbstractAction {
		AddFxSend() {
			...
			String s = i18n.getLabel("FxSendPane.AddFxSend.tt");
	...
	class RemoveFxSend extends AbstractAction {
		RemoveFxSend() {
			super(i18n.getLabel("FxSendPane.RemoveFxSend"));
			
			String s = i18n.getLabel("FxSendPane.RemoveFxSend.tt");
			putValue(SHORT_DESCRIPTION, s);
	...
	class ChannelRoutingTable extends JTable {
		private String[] columnToolTips = {
			i18n.getLabel("FxSendsPane.ttAudioSrc"),
			i18n.getLabel("FxSendsPane.ttAudioDst"),
		};
	...
	class ChannelRoutingTableModel extends AbstractTableModel implements ListSelectionListener {
		private String[] columnNames = {
			i18n.getLabel("FxSendsPane.audioSrc"),
			i18n.getLabel("FxSendsPane.audioDst")
		};
